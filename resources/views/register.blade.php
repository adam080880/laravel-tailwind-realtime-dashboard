...
@section('content')
  <div class="w-full h-svh bg-primary flex justify-center items-center">
    <div class="bg-background p-8 rounded-lg shadow-lg w-full max-w-[400px] mx-2">
      ...
      <form action="{{ route('register.api') }}" method="post" class="mt-4">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
          ...
          <input type="password" name="confirm_password" id="confirm_password" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" required>
          @if ($errors->has('confirm_password'))
            <span class="text-red-500 text-xs mt-1">{{ $errors->first('confirm_password') }}</span>
          @endif
        </div>
        <div class="flex flex-col gap-4">
          <button type="submit" class="bg-primary text-background px-4 py-2 rounded-md">Register</button>
          <a href="{{route('login')}}" class="text-center text-primary hover:underline">Login</a>
        </div>
      </form>
